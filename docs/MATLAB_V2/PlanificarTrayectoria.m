...
% FLAG KEY: 1=Corte, 2=Reposo, 3=Traslado Seguro.
    ...
    % --- 1. Definicion y Conversion de Entradas ---
    ...
    % --- 2. CONSTRUCCION DE LA TRAYECTORIA INICIAL [X Y Z FLAG V_DESEADA] ---
    ...
    for b = 1:length(idx_nan)-1
        ...
        % 2.2. Manejo de Transiciones y Enlaces (Logica de Salto Completo)
        ...
        if isempty(TrayFinal)
            ...
            % 2) TRANSICION ENTRE BLOQUES (El NaN gap) - SALTO COMPLETO
            p_prev = TrayFinal(end,1:3);    
            p_ini  = bloque(1,1:3);         
            Z_cut_prev = p_prev(3);
            Z_cut_next = p_ini(3); 
            
            % --- CORRECCION CLAVE: Punto de Transicion ---
            % Se inserta un punto identico al final del corte, pero marcado como FLAG=3.
            ...
            trans_up = [];
            if abs(Z_home - Z_cut_prev) > 1e-6 % Solo si no esta ya en Z_home
            ...
            % Concatenar: Punto de transicion (rompe color), LIFT, XY, PLUNGE
        ...
        if flag_bloque == 1 
            ...
            V_deseada(end) = 0; % Forzar detencion al final del corte
    ...
    if ~isempty(TrayFinal)
        ...
        % Asegurar que el ultimo punto sea de Reposo (FLAG=2, V=0)
        if TrayFinal(end, 4) ~= 2
             % Usamos el ultimo XY alcanzado y Z_home
...
% --------------------------------------------------------------------------------
    ...
    % 3.1. Iteracion hacia adelante (Aceleracion)
    V_perfilada(1) = 0; 
    for i = 2:N
        V_prev = V_perfilada(i-1);          
        V_target = V_deseada(i);            
        
        % Forzar V=0 en puntos de reposo o inicio/fin de corte/transicion
    ...
    % 3.2. Iteracion hacia atras (Desaceleracion)
    V_perfilada(N) = 0; 
    for i = N-1:-1:1
