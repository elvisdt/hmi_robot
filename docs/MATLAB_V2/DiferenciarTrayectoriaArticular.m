...
% DIFERENCIARTRAYECTORIAARTICULAR - Calcula Q_dot y Q_ddot usando diferencias finitas.
    ...
    if isfield(params, 'paso')
        ...
        dL_cart = 0.001;
        warning('Parametro "paso" no encontrado en params. Asumiendo 1 mm (0.001 m).');
    end
    
    if isfield(params, 'Fs')
        dt_min = 1 / params.Fs; % Usamos el periodo de muestreo si esta disponible
    else
        dt_min = 0.005; % 5 ms, una asuncion razonable
    end
    
    % --- 1. Calculo de Tiempos (dt) ---
    dL_min = dL_cart * 0.01; % Umbral de distancia minima (para puntos repetidos)
    
    for i = 2:num_puntos
        dt = dt_min; % Inicializamos con el tiempo minimo por defecto
        ...
        % 2. Manejo de puntos de detencion forzada (V=0)
        elseif V_ms(i) < 1e-6 
            % Si V_perfilada es cero (inicio/fin de corte), forzamos dt_min
            dt = dt_min; 
            
        % 3. Calculo normal de tiempo
        else
        ...
        Tiempos(i) = Tiempos(i-1) + max(dt, 1e-9); % Acumulacion del tiempo
    end
    
    % --- 2. Diferenciacion Numerica (Q_dot y Q_ddot) ---
    
    for j = 1:3 % Para cada articulacion (d1, th2, th3)
        ...
        % Calculo de Q_ddot a partir de Q_dot (mismo metodo)
    ...
    % --- 2.5. APLICACION DE LIMITES FISICOS ARTICULARES (CORRECCION CRITICA) ---
    % Esto asegura que las velocidades y aceleraciones calculadas no superen
    % las capacidades fisicas del motor (que es donde reside el cuello de botella real).
    ...
    % --- 3. Suavizado de la Aceleracion Articular (Q_ddot) ---
    % Se aplica el suavizado DESPUES de limitar, para eliminar el ruido residual.
    
    % Ventana de suavizado: 5% del total de puntos (ajustable)
    window_size = max(3, 2 * floor(num_puntos * 0.05 / 2) + 1); 
    
    for j = 1:3 
        % Aplicar filtro de media movil a Q_ddot
        Q_ddot(:, j) = smoothdata(Q_ddot(:, j), 'movmean', window_size);
    end
    
    % --- 4. Limpieza Final de Ruido Numerico ---
